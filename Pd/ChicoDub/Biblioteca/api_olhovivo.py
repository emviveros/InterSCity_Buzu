...
# Módulo api_olhovivo.py

import requests
import pd   # Biblioteca para Py4Pd
...
class OlhoVivoAPI:
    ...
    def post(self, endpoint, data={}):
        ...
        if response.status_code == 200:
            pd.print(f'Requisição POST para {endpoint} bem-sucedida!')
            return response.json()
        else:
            pd.error(f'Falha na requisição POST para {endpoint}.')
            return None
    ...
    def authenticate(self):
        ...
        """
        # pd.print('Iniciando autenticação na API Olho Vivo.')
        response = requests.post(f'{self.base_url}/Login/Autenticar?token={self.token}')
        if response.status_code == 200:
            pd.print('Autenticação bem-sucedida!')
            self.headers['Cookie'] = response.headers['Set-Cookie']
            return 1
        else:
            pd.error('Falha na autenticação.')
            return 0
    ...
    def get(self, endpoint):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                pd.error(f'Falha na requisição GET para {endpoint}. Código de status: {response.status_code}')
                return None
        except requests.exceptions.RequestException as e:
            pd.error(f'Erro na requisição GET para {endpoint}: {e}')
            return None
    ...
    def get_kmz(self, endpoint):
        ...
        response = requests.get(f'{self.base_url}/{endpoint}', headers=self.headers)
        if response.status_code == 200:
            pd.print(f'Requisição get_kmz para {endpoint} concluída com sucesso.')
            filename = endpoint.split('/')[-1] + '.kmz'
            with open(filename, 'wb') as f:
                f.write(response.content)
            return filename
        else:
            pd.error(f'Falha na requisição GET para {endpoint}.')
            return None
